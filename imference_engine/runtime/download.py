...
(``python -m imference_engine.wan.download …`` still works via a back-compat
shim in ``wan/download.py``.)
"""
...
def main(argv=None) -> None:
    ...
    if len(argv) < 2:
        sys.exit("usage: python -m imference_engine.runtime.download <url> <dest> [threads]")
